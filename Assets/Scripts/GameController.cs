        ...
        if (questController.QuestPanels.Count != 0)
        {
            foreach (QuestPanelController questPanel in questController.QuestPanels)
            {
                ...
                if (!questPanel.quest.Completed)
                {
                    ...
                    // ¬€»√–€ÿ œŒ ÷¬≈“”
                    if (questPanel.quest.winCondition == QuestController.Quest.WinCondition.SAME_COLOR_WIN) // ¬€»√–€ÿ œŒ ÷¬≈“”
                    {
                        ...
                        {
                            if (cardController.card.cardColor == questPanel.quest.WinColor) //»Ÿ≈Ã —ŒŒ“¬≈“—“¬”ﬁŸ”ﬁ  ¿–“”
                            {
                    ...
                    //¬€»√–€ÿ œŒ “»œ”
                    if (questPanel.quest.winCondition == QuestController.Quest.WinCondition.SAME_TYPE_WIN)  //¬€»√–€ÿ œŒ “»œ”
                    {
                        List<CardController> winCards = new List<CardController>();
                        CardController prevCard = null;
                        foreach (CardController cardController in CardsOnTable)
                        {
                            if (cardController.card.Type == questPanel.quest.WinningType) //»Ÿ≈Ã —ŒŒ“¬≈“—“¬”ﬁŸ”ﬁ  ¿–“”
                            {
                                if (questPanel.quest.winMethod == QuestController.Quest.WinMethod.WIN_BY_QUANTITY) //œŒ  ŒÀ»◊≈—“¬” Õ¿ œŒÀ≈
                                    winCards.Add(cardController);
                    ...
                    // œ–Œ»√–€ÿ œŒ ÷¬≈“”
                    if (questPanel.quest.loseCondition == QuestController.Quest.LoseCondition.SAME_COLOR_LOSE) // œ–Œ»√–€ÿ œŒ ÷¬≈“”
                    {
                        List<CardController> loseCards = new List<CardController>();
                        CardController prevCard = null;
                        foreach (CardController cardController in CardsOnTable)
                        {
                            if (cardController.card.cardColor == questPanel.quest.LoseColor) //»Ÿ≈Ã —ŒŒ“¬≈“—“¬”ﬁŸ”ﬁ  ¿–“”
                            {
                    ...
                    //œ–Œ»√–€ÿ œŒ “»œ”
                    if (questPanel.quest.loseCondition == QuestController.Quest.LoseCondition.SAME_TYPE_LOSE)  //œ–Œ»√–€ÿ œŒ “»œ”
                    {
                        List<CardController> loseCards = new List<CardController>();
                        CardController prevCard = null;
                        foreach (CardController cardController in CardsOnTable)
                        {
                            if (cardController.card.Type == questPanel.quest.LosingType) //»Ÿ≈Ã —ŒŒ“¬≈“—“¬”ﬁŸ”ﬁ  ¿–“”
                            {
                                if (questPanel.quest.loseMethod == QuestController.Quest.LoseMethod.LOSE_BY_QUANTITY) //œŒ  ŒÀ»◊≈—“¬” Õ¿ œŒÀ≈
                                {
                                    loseCards.Add(cardController);
                                }
